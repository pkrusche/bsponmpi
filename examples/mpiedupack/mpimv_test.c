...
void mpiinput2triple(int p, int s, int *pnA, int *pnz, 
    ...
    */
    ...
    int pA, mA, nA, nzA, nz, q, maxnz, k,
        *Nz, *Pstart, *ia, *ja, *ib, *jb;
    double *a, *b;
...
void mpiinputvec(int p, int s, const char *filename,
    ...
    */
    ...
    int n, pv, q, b, size, i, j, k, proc, nv,
        *Tmp, *Tmp2, *Tmp3, *Nsend, *Nrecv, *Offset_send, *Offset_recv, *Start,
        *Nv, *vindex;
    FILE *fp;
    ...
    for (q=0; q<p; q++){
        ...
        if(s==0){
            ...
            for(k=q*b; k<(q+1)*b && k<n; k++){
                ...
                Tmp[j]= proc; j++;     /* processor */ 
                Tmp[j]= i;    j++;     /* global index */       
                Tmp[j]= Nv[proc]; j++; /* local index */
                Nv[proc]++;
            }
        }
        /* Send size triples to each processor. To make the number
           of sends equal, some dummy triples may be sent. */
        MPI_Scatter(Tmp,3*size,MPI_INT,
                    &Tmp2[q*3*size],3*size,MPI_INT,0,MPI_COMM_WORLD);
    }
    ...
    MPI_Alltoallv(Tmp, Nsend,Offset_send,MPI_INT,
                  Tmp3,Nrecv,Offset_recv,MPI_INT,MPI_COMM_WORLD);
...
int main(int argc, char **argv){
    ...
    for(iter=0; iter<NITERS; iter++)
        mpimv(p,s,n,nz,nrows,ncols,a,ia,srcprocv,srcindv,
